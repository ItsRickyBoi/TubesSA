...
def display_results(result_str):
    ...
    window_height = min(400, max(150, 18 * lines))  # Calculate the height based on the number of lines
    result_window.geometry(f"500x{window_height}")  # Dynamic window size
    tk.Label(result_window, text=result_str, justify=tk.LEFT, wraplength=480).pack(pady=10, padx=10)

def find_palindrome():
    user_input = entry.get()
    if any(char.isdigit() for char in user_input):  # Check if the input contains any digits
